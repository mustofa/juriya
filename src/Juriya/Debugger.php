...
class Debugger {
	...
	{
		// Resolve tunnel
		self::_resolveTunnel() and $tunnel = self::$tunnel;
	...
	public static function trace($type = '', $message = '', $stack = array())
	{
		...
		if ($tunnel == 'CLI') {
			$output  = self::getColoredString(str_pad('TYPE ', 7, " ", STR_PAD_LEFT), 'yellow');
			$output .= self::getColoredString($type, 'cyan') . "\n";
			$output .= self::getColoredString(str_pad('MSG ', 7, " ", STR_PAD_LEFT), 'yellow');
			$output .= self::getColoredString($message, 'cyan') . "\n";
		} else {
		...
		if ( ! empty($stack)) {
			...
			foreach ($stack as $index => $item) {
				...
				if ($tunnel == 'CLI') {
					$itemNode .= "\n";
					$itemNode .= self::getColoredString(str_pad('#' . ($index + 1), 10, ' ', STR_PAD_LEFT),
					                                    'green');
					$itemNode .= "\n";
				}
				...
				foreach ($item as $head => $body) {
					...
					if ($tunnel == 'CLI') {
						$itemNode .= self::getColoredString(str_pad(strtoupper($head), 10, " ", STR_PAD_LEFT), 
						                                    'light_green'); 
	    				$itemNode .= " => " . self::getColoredString($body, 'cyan') . "\n";
					} else {
			...
			if ($tunnel == 'CLI') {
				...
				$output .= self::getColoredString(str_pad('STACK ', 7, " ", STR_PAD_LEFT), 'yellow') 
				           . self::getColoredString(count($formatedStack) . ' level(s)', 'cyan') . "\n"
				           . "\n" . self::getColoredString('#START', 'green')
				           . "\n" . implode('' , $formatedStack) . "\n"
				           . self::getColoredString('#END', 'green');
			} else {
