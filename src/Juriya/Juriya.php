...
class Juriya {
	...
	function __construct(Array $bootstrap, Collection $config)
	{
		...
		if ($config->valid() && self::initStatus() == FALSE) {
	...
	public static function configure(Array $configs)
	{
		...
		$configs = array_map(function ($item) use (&$configs) {
		    ...
		    // Scan the level and inspect matched cursor
	...
	public function execute()
	{
		...
		$config->set('controller', PATH_SYS . 'Controller' . EXT);
	...
	public static function autoload($item)
	{
		...
		foreach ($paths as $path) {
			...
			if (($classFile = $file) and file_exists($classFile)) {
				...
				continue;
			} elseif ($classFile = str_replace('_', DIRECTORY_SEPARATOR, $file) and file_exists($classFile)) {
				include_once $classFile;
				$loaded = TRUE;
